...
The gateway is required execution infrastructure for command-backed agent work.
It is the "hands" of the system: the LLM can propose meaning, the runtime can
validate contracts and approval policy, but the gateway is the process that
actually touches the shell, terminal, working directory, and local tools on a
machine.

Install and verify a gateway before using the agent for local execution:
...
`/exec/cancel`, `/terminal/ws`, and gateway-managed local LLM routes under
`/llm/*`. The Agent UI proxies the LLM launch controls through
`/api/agent/llm/status`, `/api/agent/llm/start`, `/api/agent/llm/stop`, and
`/api/agent/llm/log`.
...
The SQL agent is a specialized capability path for database questions. It uses
Parameter Store database profiles, stored schema discovery metadata, and the
same request clarification/confirmation policy as the rest of the runtime.
...
Streaming decomposition changes how evidence is gathered, not which actions
are trusted. Agentic mode validates and executes one scoped task/action at a
time, then feeds real outputs into the next step. Every step still uses the
same command contract and safety checks.
...
Parameter Store `context_json` follows the same rule. Domain guidance can help
the model choose tables, files, metrics, or concepts, but it cannot lower risk,
approve joins, reveal credentials, skip confirmation, or bypass hard
validation.
...
and policy checks still do not yield to the LLM or the UI profile.
...
## SQL Agent, Discovery, And Join Validation

Database profiles live in the Parameter Store. Executable connection data and
generated SQL discovery metadata live in `value_json`; non-secret domain notes
live in `context_json`.

`/discoverdb` performs schema discovery on each run and returns reviewable
create/update drafts. On commit it writes these generated fields into
`value_json`:
...
Existing `context_json` is preserved across refresh. If a database schema
changes, run `/discoverdb` again and commit the update.

The SQL validator keeps hard safety checks strict:

- unknown table or column;
- parse failure or multiple statements;
- non-read-only SQL without confirmation;
- dangerous mutation;
- row-returning query without a valid limit.

Join validation is intentionally more permissive for read-only queries:

| Join Evidence | Validator Behavior |
| --- | --- |
| DB-declared foreign key | Approved silently. |
| Inferred secondary key, DICOM/domain context, or user semantic guidance | Allowed with `join_relationship_warnings`. |
| No known FK or semantic evidence | Allowed with a louder warning. |

Only database-declared foreign keys count as approved joins. User/domain notes
and inferred joins are prompt context, not join approval. Read-only SQL with
warnings executes normally; mutating SQL still goes through confirmation.
...
Stepwise execution is streamed through visible trace and command events so the
UI can show each scoped action, its output, and any repair or approval state.
