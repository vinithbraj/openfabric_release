...
| --- | --- | --- | --- |
| API layer | `src/agent_runtime/api/` | HTTP routes, request state, streaming and non-streaming integration responses, confirmation/clarification resume, settings preferences, memory/events/tasks/monitors/gateway endpoints | semantic interpretation |
| Agent UI assets | `src/agent_runtime/api/static/agent_ui/` | browser UI, chat controls, desktop/client/mobile surfaces, all desktop drawers, trace/visualization display, terminal, command capsules | runtime trust decisions |
...
| Parameter Store | `src/agent_runtime/parameters/` | executable parameter values, non-secret `context_json`, matching, masking, audit, shell env derivation | revealing secrets to prompts or bypassing validators |
| Learning runtime caches | `src/agent_runtime/lrn_total_tasks/`, `plan_cache/`, `command_template_cache/`, `computation_cache/` | private LRN-T task structures, plan/cache stats, LR Direct and LR-EX replay metadata | weakening validation, approval, or current memory directives |
...
- expose `/healthz`;
- serve `/agent-ui`, `/agent-ui-client`, `/agent-ui-mobile`, `/directory`,
  `/settings`, `/prompt-editor`, `/parameter-editor`, `/learning-ledger`, and
  `/reliability`;
- accept `/api/agent/request`;
- stream request events and command output;
- expose non-streaming `/api/agent/integrations/...` execution envelopes for
  external systems while reusing the same request lifecycle and persisted
  runtime controls;
...
- expose Settings, UI preferences, Memory, Parameter Store, Events, Tasks,
  Monitors, Notifications, Gateway, model, terminal, runtime-control, cache,
  LRN-T/LR-T, LR Direct, learning-ledger, reliability, and restart endpoints;
- serve `/parameter-editor` and `/prompt-editor?mode=parameters`;
...
- show separate resizable Settings, Memory, Gateway, Events, Tasks, Monitors,
  Parameters, Chat History, and Notifications drawers;
- show Directory, Mission Control Settings, Prompt Editor, Learning Ledger,
  Reliability, Client UI, and Mobile UI surfaces that share the same static
  frontend conventions;
- show Parameter Store quick edit controls and Open Editor actions for the
  full Parameter Editor;
- persist durable UI preferences through backend settings while keeping drawer
  widths and transient layout hints in browser storage;
- keep quick controls and the Settings drawer reachable in immersive mode;
...
- mirror backend UI preferences and keep local ergonomics in browser storage.
...
- attach matched Parameter Store summaries, execution-only values, shell env
  names, and bounded `context_json` guidance;
- look up and write LRN-T/LR-T total-task structures around decomposition;
...
- execute decomposed tasks step by step, feeding real evidence into the next
  operator decision;
- reuse exact LR Direct streaming-step cache entries when current context
  matches;
- use LR-EX semantic shape judging for equivalent payload-aware command/Python
  replay shapes when exact-step text does not match;
...
commands directly and cannot bypass hard safety. Mutating work pauses for
approval, and every decomposed step still goes through the same safety policy,
command deny rules, cwd validation, binding validation, and policy checks.
...
**Directory:** `src/agent_runtime/parameters/`

### Responsibilities

- initialize and migrate the SQLite schema;
- store Parameter Store records with `value_json` and `context_json`;
- mask sensitive values in list/get summaries;
- reveal raw `value_json` only through the audited reveal path;
- derive shell environment variables and execution values from `value_json`;
- keep `context_json` out of raw shell/execution payloads;
- retrieve relevant parameters by key, aliases, tags, value schema, database
  profile, and context text;
- expose bounded context summaries for prompts and richer matched context for
  clarification resolution;
- support draft/create/update/delete flows for the parameter capability and UI.

### Boundary

Parameter context is advisory. It can help the LLM choose a database, concept,
metric, file, or service, but it cannot reveal secrets, approve joins, lower
risk, skip confirmations, or override validators.


## Learning Runtime Caches

**Directories:** `src/agent_runtime/lrn_total_tasks/`,
`src/agent_runtime/command_template_cache/`,
`src/agent_runtime/computation_cache/`

### Responsibilities

- store successful LRN-T total-task structures for later LR-T lookup;
- match total-task entries by prompt shape, classification snapshot, model
  family, workflow mode, and registry contract hash;
- quarantine total-task entries that fail validation or produce failed reuse;
- store command-template and computation replay entries for LR Direct;
- store payload-binding metadata for LR-EX replay;
- shortlist semantically plausible LR-EX candidates for the structured shape
  judge;
- expose stats/clear APIs for plan/computation caches, LRN-T, and LR Direct.

### Boundary

Learning caches are local accelerators. They do not replace live validation,
memory compliance, approval, gateway routing, or execution safety. They should
not be tracked as portable seed artifacts unless explicitly curated.
...
- shape final-answer learning summaries into concise LR-T, LR-D, and LRN
  groups instead of raw event/id dumps;
...
- release active non-task request tracking after the root request or any child
  continuation is no longer pending, so queued durable tasks resume at the
  right time;
...
## Durable Task Queue And Auto-Follow

**Main files:** `src/agent_runtime/api/agent_ui_support/route_groups/task_monitor_shared.py`,
`src/agent_runtime/tasks/store.py`, `src/agent_runtime/api/static/agent_ui/app.js`

### Responsibilities

- store queued composer prompts as durable tasks when the composer Run button
  is pressed while another request, task, approval, or clarification is active;
- preserve chat source, conversation id, selected `llm_model`, agent mode,
  gateway/routing metadata, settings context, terminal context, and
  auto-approve intent in task context;
- start queued tasks only when the active request/task gate is clear;
- record attempts, checkpoints, parent request ids, child continuation request
  ids, final previews, and blocker reasons;
- let the open UI fetch the retained trace before opening a live stream;
- follow the attached durable task when background auto-approval moves from a
  parent approval request to a child execution request;
- retire stale UI approval controls once task-owned auto-approval has continued
  the run.

### Boundary

Durable task queuing changes when work starts and how it survives UI refreshes.
It does not create a second execution path, skip validation, bypass approval
policy, or replace the task drawer's explicit Create Task and Create + Start
controls.
